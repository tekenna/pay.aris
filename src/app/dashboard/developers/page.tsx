      ...
      >
        <div className="grid gap-6">
          <section className="rounded-[18px] bg-slate-50 p-5">
            ...
            <pre className="mt-4 overflow-x-auto rounded-[16px] bg-slate-950 p-5 text-sm text-slate-100"><code>{`curl -X POST "${process.env.NEXT_PUBLIC_API_BASE_URL || "https://api.ariswallex.com/api"}/checkout/create" \\
      ...
      "email": "john@example.com"
          ...
          <section className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-[18px] bg-slate-50 p-5">
              ...
              <pre className="mt-4 overflow-x-auto rounded-[16px] bg-slate-950 p-5 text-sm text-slate-100"><code>{`const response = await fetch("${process.env.NEXT_PUBLIC_API_BASE_URL || "https://api.ariswallex.com/api"}/checkout/create", {
  method: "POST",
  headers: {
...
console.log(payload.data.checkout_url);`}</code></pre>
            ...
            <div className="rounded-[18px] bg-slate-50 p-5">
              ...
              <pre className="mt-4 overflow-x-auto rounded-[16px] bg-slate-950 p-5 text-sm text-slate-100"><code>{`curl "${process.env.NEXT_PUBLIC_API_BASE_URL || "https://api.ariswallex.com/api"}/payments/verify/ARIS_CHK_REFERENCE"`}</code></pre>
              <p className="mt-4 text-sm leading-7 text-slate-600">
